...
def get_document_no_date_pdf(pdf_text: str) -> tuple:
    ...
    doc_no_mask = (
        ...
        r"АКТ[\s.]ПРИЕМА-ПЕРЕДАЧИ\s\(ПОСТАВКИ\)\sМОЩНОСТИ\s№\s([\d]*?)\s*ОТ\s*(\d{2}\s[А-Я]+\s\d{4})\s",
        r"ПРИЕМА\s?[-–]\s?ПЕРЕДАЧИ\s\(ПОСТАВКИ\)\sМОЩНОСТИ\s№\s([\d-]*?)\s*ОТ\s*(\d{2}\.\d{2}\.\d{4})\s",
        r"АКТ[\s.]*?ПРИЕМА-ПЕРЕДАЧИ\s\МОЩНОСТИ\s№\s([\d-]*?)\s*ОТ\s*(\d{2}\s[А-Я]+\s\d{4})",
        r"АКТ[\s.]ПРИЕМА-ПЕРЕДАЧИ\s№\s([\d-]*?)\s*ОТ\s*(\d{2}\s[А-Я]+\s\d{4})",
        r"АКТ[\s.]ПРИЕМА\s*?[-–]\s*?ПЕРЕДАЧИ\s[\w\W]{1,100}№\s?([\d\-]+)\sОТ\s(\d{2}\.\d{2}\.\d{4})",
        r"АКТ\s*?ПРИЕМА-ПЕРЕДАЧИ\sЭЛЕКТРИЧЕСКОЙ\sЭНЕРГИИ\s*?№\s*?([\d]*?)\s*ОТ\s*(\d{2}\.\d{2}\.\d{4})\s",
        r"АКТ\s*?ПРИЕМА\s*?[-–]\s*?ПЕРЕДАЧИ\s[\w\W]*?№\s?([\w\-\/]+)\sОТ\s(\d{2}\.\d{2}\.\d{4})\s",
        r"ПРИЕМА\s?[-–]\s?ПЕРЕДАЧИ\s*?МОЩНОСТИ\s№\s([\d-]*?)\s*ОТ\s*(\d{2}\.\d{2}\.\d{4})\s",
        r"АКТ[\s.]*?ПРИЕМА-ПЕРЕДАЧИ\s\МОЩНОСТИ[\s.]*?№\s([DVR\d-]*?)\s*ОТ\s*(\d{2}\s[А-Я]+\s\d{4})",
        r"АКТ[\s.]ПРИЕМА\s*?[-–]\s*?ПЕРЕДАЧИ\s[А-Я]{1,100}\s№\s?([А-Я\d\-\_\/]+)\sОТ\s(\d{2}\.\d{2}\.\d{4})",
    )
...
def pack_and_move_sbis(_doc_file: str, _dest_path: str):
    ...
    if _short_file_name.upper().startswith(
            "DP_REZRUISP"
    ) or _short_file_name.upper().endswith(".XLS"):
        _doc_type = "ДОП"
    else:
        _doc_type = get_property_from_xml(_doc_file, "Документ", "Функция").upper()
                ...
                if os.path.isfile(full_doc_file):
                    ...
                    if doc_file.upper().startswith(
                        ...
                        is_success = is_success and _result
                            ...
                            and not "ФАКТУРА" in doc_file.upper()
                            and "ПЕЧАТНАЯ ФОРМА" in doc_file.upper()
                            and not "ON_NSCHFDOPPR" in doc_dir.upper()
                    ):
            ...
            for archive_file in os.listdir(full_supplier_path):
                ...
                if (
                    ...
                    if is_success:
                        ...
                        full_archive_file
                ) and archive_file.upper().startswith(
                    "ПОСТУПЛЕНИЯ"
                ):  # Это СБИС:
                    is_success = True
                    # если в папке есть не обработанные архивы,
